...
<p align="center">
  A premium visual reskin of the Odoo 18 backend — persistent app sidebar, light/dark mode,
...
- They keep that styling when the `website` module is installed. `website` replaces the *root* of
  `web.login_layout` at priority 20, which discards anything an inherit of that template had just
  added: the body class every rule in `auth_pages.scss` is scoped to, the saved colours and the
  dark-mode script all vanish, and the login screen comes back in the website theme's palette. So
  the page chrome is attached one level up instead, on `web.frontend_layout` — the one node both
  roads pass through, since `website.layout` inherits `portal.frontend_layout`, which inherits
  `web.frontend_layout`. That lands whether or not `website` is installed, and needs no dependency
  on it. The cost is that the template is evaluated for every portal and website page too, so
  everything in it is gated on the request path: nothing but three cheap `t-set` lines runs
  anywhere else.
- With `website` installed the stock card is gone as well — what survives is a bare
  `oe_website_login_container` holding the form — so the logo and heading block is emitted as its
  immediately preceding sibling and `auth_pages.scss` draws the two as one panel. The website's own
  header and footer are left in place there: installing `website` is a decision to put the site's
  chrome on its public pages, and the login page is one of them.
...
2. **Bootstrap runtime variables** — retargeted at document and component level. Odoo 18 Community
