...
<h1 align="center">BlueNova — Odoo 18 Backend Theme</h1>

<p align="center">
  <img src="https://img.shields.io/badge/Odoo-18.0-714B67?logo=odoo&logoColor=white" alt="Odoo 18.0">
...
<p align="center">
  A premium glossy metallic reskin of the Odoo 18 backend — deep royal blue, electric blue and
  chrome-silver accents, a persistent app sidebar, light/dark mode, a themed login/signup/
  reset-password flow, an optional public home page, a KPI landing dashboard, and an in-app
  Settings screen for recolouring it all without touching SCSS.
...
| **Palette** | Deep Royal Blue (`#003087`) and Electric Blue (`#00AEEF`) with cool silver/chrome gradients over tiered white surfaces — fully overridable per-instance from Settings. |
| **Shape & elevation** | Rounded corners, soft multi-layer shadows, and glossy, glassmorphic panels, cards and dropdowns. |
...
- **Bundled app artwork** — 59 single-colour icons covering the standard Odoo apps, painted through
...
- **Control panel as a page header**, retinted top navbar, and glassmorphic kanban boards.
- **Responsive** — tablet and phone breakpoints; below `md` the rail steps aside for Odoo's own
  slide-in app menu.
...
- An uploadable **login background image** (light and dark), stored as `ir.attachment` records
  rather than in `ir.config_parameter`, so no base64 payload rides along on every request. The
  **sidebar brand image** behind it is no longer on this screen — the sidebar rule that painted it
  was dropped — but the field and its attachment stay, because the login page still serves that
  image as its logo when "Use Brand Image As Login Logo" is ticked.
...
- **Export / Import / Reset** — the current palette can be downloaded as a JSON preset, re-imported
  later through `wizard/theme_import_wizard.py` (validated field-by-field before anything is
  written), or reset back to the compiled defaults in one click.
...
| Optional | `crm` — `views/crm_lead_views.xml` restyles the CRM pipeline kanban card, but it ships **inactive**: it is not registered in the manifest's `data` list because loading it would make the theme hard-depend on `crm` (see [Project structure](#project-structure)). Every other feature works without `crm` installed. |
...
git clone https://github.com/NandiniGohel/Odoo18-BlueNova-Backend-Theme.git bluenova_backend_theme
...
— no code change or asset rebuild required. See [Settings → Theme Settings](#settings--theme-settings)
above.
...
```scss
:root {
    --cmt-primary: #003087;          // deep royal blue
    --cmt-tertiary: #00aeef;         // electric blue accents
...
Apps with no entry fall back to their own Odoo icon, then to a neutral placeholder.
...
### Enable the CRM pipeline card restyle

`views/crm_lead_views.xml` is shipped but not loaded. To turn it on, add `crm` to `depends` and
the file to `data` in `__manifest__.py`, then update the module.
...
│   └── crm_lead_views.xml             # optional CRM pipeline card restyle — not loaded by default
└── static/
    ├── description/                   # app-store card: icon, banner, index.html
    └── src/
        ├── fonts/                     # Inter + Poppins, latin subset
        ├── image/icons/               # 59 bundled app icons for the sidebar
        ├── scss/
        ...
        │   ├── settings_page.scss
        │   ├── buttons_misc.scss
        │   ├── brand_bridge.scss      # core's compiled brand literals → the theme's tokens
        │   ├── responsive.scss        # width / height / pointer-capability rules
        ...
        │   └── crm_pipeline_stats*.js     # CRM KPI banner — bundled but unwired, see note below
        └── xml/                        # OWL templates + inherited core templates
```

> `crm_pipeline_stats.js` / `crm_pipeline_stats_patch.js` and `views/crm_lead_views.xml` ship in
> the module but are **not** registered in `__manifest__.py`'s `assets` or `data` sections, so the
> CRM pipeline restyle is inert out of the box. This keeps the theme's only hard dependencies at
> `web` and `base_setup`. Wire them in (and add `crm` to `depends`) if you want the feature — see
> [Enable the CRM pipeline card restyle](#enable-the-crm-pipeline-card-restyle).
...
2. **Bootstrap runtime variables** — retargeted at document and component level.
...
- The CRM pipeline card restyle ships disabled by default (see [Project structure](#project-structure)).
