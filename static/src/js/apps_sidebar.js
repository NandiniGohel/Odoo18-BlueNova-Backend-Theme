 ...
 * trigger_up. Odoo 18 has no widget tree and no core.bus: this is a plain
...
export class AppsSidebar extends Component {
    ...
    setup() {
        ...
        // The active company lives on the `company` service on 18.0. It was
        // only folded into `user` (as user.activeCompany) in 19.0, so reading
        // it from there here would be `undefined` and the workspace header
        // would silently fall back to the database name.
    ...
    get workspace() {
        ...
        return {
            name: (company && company.name) || session.db || _t("Workspace"),
     ...
     * Built exactly the way NavBar.getMenuItemHref builds it — Odoo 18
